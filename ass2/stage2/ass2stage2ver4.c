...
stage_2(records_t records[], int valid_lines) {
    int count[MONTHS], start_year[], end_year;
    double rainfallarray[valid_lines];

    check_month(records, valid_lines, count, start_year, end_year, rainfallarray, STAGE_2);
    return;
}
...
/* This function does all the work for stage 2 and stage 3; checks and combines
 ...
 stage 3 */
void
check_month(records_t records[], int valid_lines, int *count, int *start_year, int *end_year, double *rainfallarray, int stage) {
    int i, j, k;
    for (i=0; i<=MONTHS; i++) {
        count[i] = 0;
    }
    ...
    for (i=JAN; i<=MONTHS; i++) {
        ...
        if (stage == STAGE_2) {
            median = median_rainfall(rainfallarray, count[i]);
        }
