...
#include <stdio.h>
#include <stdlib.h>
#include <math.h> /* for celing functions */
...
void stage_4(records_t records[], int valid_lines, int year);

/* function prototypes */
int mygetchar(void);
void check_month(records_t records[], int valid_lines, int stage, int *count, int *start_year, int *end_year, rainfall_t rainfallarray[]);
...
void mean_array(records_t records[], int valid_lines, double *mean);
...
main(int argc, char *argv[]) {
	int i = 0, j, hcheck, valid_lines = 0, year;
    ...
    printf("\n");
    /* Now start stage 4 output, we use a for loop to look at all command line
    arguments provided. I HAVE ASSUMED ONLY VALID YEARS WILL BE INPUT */
    for (j=FIRST; j<argc; j++) {
        year = atoi(argv[j]);
        stage_4(records, valid_lines, year);
    }
    /* bada bing bada boom we are finished */
    printf("Ta daa!\n");
...
stage_2(records_t records[], int valid_lines) {
    int i, count[MONTHS], start_year[MONTHS], end_year[MONTHS];
    double mean[MONTHS];
    rainfall_t rainfallarray[MONTHS];


    check_month(records, valid_lines, STAGE_2, count, start_year, end_year, rainfallarray);

    for (i=1; i<=MONTHS; i++) {
        if (count[i] != 0 ) {
            mean[i] = mean_rainfall(rainfallarray[i].rainfallarraystruct, count[i]);
            printf("S%d, %s, %2d values, %4d-%4d, mean of %5.1lfmm\n", STAGE_2,
                months[i], count[i], start_year[i], end_year[i], mean[i]);
        }
        else {
...
stage_3(records_t records[], int valid_lines) {
    ...
    check_month(records, valid_lines, STAGE_3, count, start_year, end_year, rainfallarray);

    for (i=JAN; i<=MONTHS; i++) {
        if (count[i] != 0 && count[i] != 1) {
            tau[i] = compute_tau(rainfallarray[i].rainfallarraystruct, count[i]);
            printf("S%d, %s, %2d values, %4d-%4d, mean of %5.1lfmm\n", STAGE_3,
                months[i], count[i], start_year[i], end_year[i], tau[i]);
        }
        else {
...
/* stage 4 format */
void
stage_4(records_t records[], int valid_lines, int year) {
    int i, count[MONTHS], start_year[MONTHS], end_year[MONTHS];
    double mean[MONTHS];
    rainfall_t rainfallarray[MONTHS];

    /* fill an array with mean values */
    check_month(records, valid_lines, STAGE_4, count, start_year, end_year, rainfallarray);
...
/*This function does all the work for stage 2 and stage 3; checks and combines
 ...
 stage 3 */
void
check_month(records_t records[], int valid_lines, int stage, int *count, int *start_year, int *end_year, rainfall_t rainfallarray[]) {
    int i, j, k;
    ...
    for (i=JAN; i<=MONTHS; i++) {
        ...
        for (j=0; j<valid_lines; j++) {
            ...
            if (records[j].month == i) {
                ...
                end_year[i] = records[j].year;
                /* array to record all rainfall for each month */
                rainfallarray[i].rainfallarraystruct[k] = records[j].rainfall;
                /* increase k */
                k++;
                /* check to find the start year */
                if (count[i] == FIRST) {
                    start_year[i] = records[j].year;
                }
            }
        }
...
do_stage4(records_t records[], int valid_lines, int year, int stage,
    ...
    /*set the empty array to zero to start with in the event of missing data*/
    for (i=JAN; i<=MONTHS; i++) {
        month_rainfall[i] = 0;
    }
...
/* prints all elements and structure of the graph required in stage 4 */
void
    ...
    for (i=max_graph_value; i>0; i-=scale) {
        ...
        for(j=JAN; j<=MONTHS; j++) {
            ...
            if ((month_rainfall[j] > i - scale) && (month_rainfall[j] <= i)) {
                ...
                /* else if not just print the last two digits of the year so
                long as it is not empty data */
    ...
    /* else; find when max_graph_value is the lowest scale value above the
     maximum rainfall */
        ...
        for (i=0; i<MAX_ROWS; i++) {
            if (maximum < max_graph_value - scale) {
...
calc_scale(double maximum) {
    ...
    double calculation = maximum / MAX_ROWS;
...
compute_tau(double rainfallarray[], int count) {
    ...
    for (i=0; i<=n-2; i++) {
        for (j=i+1; j<=n-1; j++) {
            ...
            /*everything else is +0 or in other words do nothing to delta*/
        }
    }
