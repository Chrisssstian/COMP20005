...
#define STAGE_3 3
#define STAGE_4 4
/* ---------- Flags to tell function if it is out/ret battery ---------- */
#define BATTERYOUT 0
#define BATTERYRET 1
...
int mygetchar(int hcheck);
double centroid(double coord[], int valid_lines);
void optimise(int valid_lines, double trip_batt[], double dist[], double ran[],
    double batt_out[], double batt_ret[], double time[], int stage);
...
void pop_arrays(double X[], double Y[], double KG[], int stage,
...
main(int argc, char **argv) {
    ...
    /* checks and skips header line of file without need for given function */
...
stage_1(double X[], double Y[], double KG[], int valid_lines) {
    ...
    printf("S%d, total to deliver: %5.2lf kg\n",
        STAGE_1, sum_array(KG, valid_lines));
    return;
}
/* ------------------------------ STAGE 2 ------------------------------ */
void
stage_2(double X[], double Y[], double KG[], int valid_lines) {
    ...
    pop_arrays(X, Y, KG, STAGE_2, valid_lines, dist, ran, batt_out, batt_ret,
        time, trip_batt);
    /* ------------------------------------------------------------ */
    int i;
    double sum = 0, total_dist, total_time;
    int batteries = 1; /* always will have the starting battery */
    /* check if battery is needed as well as print all necesaary data */
...
stage_3(double X[], double Y[], double KG[], int valid_lines) {
    ...
    double dist[MAX_LINES], ran[MAX_LINES], batt_out[MAX_LINES],
        batt_ret[MAX_LINES], time[MAX_LINES], trip_batt[MAX_LINES];
    pop_arrays(X, Y, KG, STAGE_3, valid_lines, dist, ran, batt_out, batt_ret,
        time, trip_batt);
    /* ------------------------------------------------------------ */
...
stage_4(double X[], double Y[], double KG[], int valid_lines) {
    ...
    double dist[MAX_LINES], ran[MAX_LINES], batt_out[MAX_LINES],
        batt_ret[MAX_LINES], time[MAX_LINES], trip_batt[MAX_LINES];
    pop_arrays(X, Y, KG, STAGE_4, valid_lines, dist, ran, batt_out, batt_ret,
        time, trip_batt);
    /* ------------------------------------------------------------ */
...
void pop_arrays(double X[], double Y[], double KG[], int stage,
     int valid_lines, double *dist, double *ran, double *batt_out,
      double *batt_ret, double *time, double *trip_batt) {
    /* ---------- Check for a package that is too heavy ---------- */
    int i;
    for(i=0; i<valid_lines; i++) {
        if (KG[i] > MAX_CARRY) {
    ...
    if (stage == STAGE_2 || stage == STAGE_3) {
        distance(dist, valid_lines, ORIGIN, X, ORIGIN, Y);
    ...
    /* ---------- Last check no package takes up >100 percent ---------- */
    for (i=0; i<valid_lines; i++) {
...
void
optimise(int valid_lines, double trip_batt[], double dist[], double ran[],
        double batt_out[], double batt_ret[], double time[], int stage) {
    /*iteration, sum and count varuables, (batteries start at 1 for
...
/* Populates the battery out/battery return of a specific delivery and uses a
 flag variable to know if it is a out/ret array being input*/
void
...
/* Returns a sum of all array elements of input */
double
