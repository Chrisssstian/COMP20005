...
void stage_2(int valid_lines, double dist[], double ran[], double batt_out[],
    double batt_ret[], double time[], double trip_batt[]);
void stage_3(int valid_lines, double dist[], double ran[], double batt_out[],
    double batt_ret[], double time[], double trip_batt[]);
...
void
stage_2(int valid_lines, double dist[], double ran[], double batt_out[],
        double batt_ret[], double time[], double trip_batt[]) {
    int i;
...
void
stage_3(int valid_lines, double dist[], double ran[], double batt_out[],
        double batt_ret[], double time[], double trip_batt[]) {
...
/* prints out structure of data for stage 4, and populates arrays for use
using new centroid*/
void
stage_4(double X[], double Y[], double KG[], int valid_lines) {
    ...
    /* call centroid function to save x and y coordinates */
...
optimise(int valid_lines, double trip_batt[], double dist[], double ran[],
    ...
    int flag_array[MAX_LINES]; /* to check if current element has been used */
    double sum = 0;
    ...
    for (j=0;j<valid_lines;j++) {
        ...
        /* check to see if current j element has been used already and to force
         next iteration if it has */
        if (flag_array[j] == 1) {
            continue;
        }

        /* check to see if current j element has not been used already and
         to change to used if executed */
        if (flag_array[j] == 0) {
            printf("S%d, package=%3d, distance=%6.1lfm, battery out=%4.1lf%%,"
