...
#       StudentID: 832391
#        version: 2 works for both drones0/1 fully testing code still here
======================================================================*/
...
optimise(int valid_lines, double trip_batt[], double dist[], double ran[], double batt_out[], double batt_ret[], double time[]) {
    ...
    for (j=0;j<valid_lines;j++) {
        printf("j = %d\n", j);
        if (new_array[j] == 1) {
            continue; /* forces next iteration as this element already used */
        }
        ...
        printf("valid = %d j = %d k = %d\n", valid_lines, j, k);
        /*if (j == valid_lines-1 && k == valid_lines) {
            sum = 0;
        }*/
